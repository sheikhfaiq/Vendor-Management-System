...
async function main() {
  ...
  console.log('🏢 Seeding Vendors, service mappings, and team members...');
  ...
  for (const vData of vendorsData) {
    ...
    // Sequence code directly set
    const vendorCode = `KSA-${String(nextCode++).padStart(4, '0')}`;
    ...
    if (vData.vendorType === VendorType.COMPANY) {
      await prisma.teamMember.create({
        data: {
          vendorProfileId: profile.id,
          name: `Engineer Ahmad (Civil)`,
          phone: `+96650${Math.floor(1000000 + Math.random() * 9000000)}`,
          email: `ahmad@${vData.email.split('@')[1]}`,
          nationality: 'Saudi',
          iqamaNumber: `10${Math.floor(10000000 + Math.random() * 90000000)}`,
          expiryDate: new Date('2028-12-31'),
          iqamaProfession: 'Civil Engineer',
          iqamaCompanyName: vData.companyName,
          bloodGroup: 'O+',
        },
      });
      ...
      await prisma.teamMember.create({
        data: {
          vendorProfileId: profile.id,
          name: `Supervisor Muhammad`,
          phone: `+96650${Math.floor(1000000 + Math.random() * 9000000)}`,
          email: `muhammad@${vData.email.split('@')[1]}`,
          nationality: 'Pakistani',
          iqamaNumber: `10${Math.floor(10000000 + Math.random() * 90000000)}`,
          expiryDate: new Date('2027-06-30'),
          iqamaProfession: 'Site Supervisor',
          iqamaCompanyName: vData.companyName,
          bloodGroup: 'A+',
        },
      });
    ...
    console.log(`  Seeded Vendor user: ${vData.email} (Completion: ${completion}%) with code ${vendorCode}`);
  }
